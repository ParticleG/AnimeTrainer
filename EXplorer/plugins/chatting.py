...
async def motivation(session: CommandSession):
    ...
    if len(args) == 0:
        ...
        motivation_figure = cq_at_parser(
            session.event.user_id) + luck_str + cq_image_parser(
...
async def couple(session: CommandSession):
    ...
    if len(args) == 0:
        ...
        try:
            ...
            couple_user_id = \
                group_member_list[int(user_md5, base=16) % len(group_member_list)][
                    'user_id']
            couple_user_str = cq_at_parser(couple_user_id)
            response = user_str + '你没有今日CP，快和VTB贴贴去吧！'
            if couple_user_id == session.event.user_id:
                for temp_group in GROUPS:
                    if session.event.group_id == temp_group['group'] and \
                            temp_group['channel'] is not None:
